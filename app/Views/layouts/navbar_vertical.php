    ...
    <ul
        ...
        <li>
            <div class="user-view" style="">
                ...
                <a href="#user" style="margin-right: 0px;"><img class="circle"  style="width: 50px; height:50px;" src="<?= session('user') && session('user')->photo ? base_url(['assets/img/users', session('user')->photo]) : base_url().'/assets/img/'.'user.png' ?>"></a>
        ...
        <?php foreach (menu('Pagina') as $item): ?>
            <li class="bold <?= isActive(urlOption($item->id)); ?>"><a
                        class="waves-effect waves-cyan  <?= isActive(urlOption($item->id)); ?> <?= countMenu($item->id) ? 'collapsible-header' : ''; ?>"
